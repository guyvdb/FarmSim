...
namespace fsim {
    ...
    GamePlayState::GamePlayState(Game *game) : mCircle(150.0f) {
        this->mGame = game;
        sf::Vector2f pos = sf::Vector2f(this->mGame->window()->getSize());
        this->mGameView.setSize(pos);
        pos *= 0.5f;
        this->mGameView.setCenter(pos);


        this->mCircle.setFillColor(sf::Color::Red);


        // start the simulation 
        this->mGame->StartSimulation();
    ...
    void GamePlayState::draw(sf::Time dt) {
        this->mGame->window()->setView(this->mGameView);
